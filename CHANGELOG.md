...
- **Optional `dialback/browsermesh` transport**: a new, additive-only module (`transports/browsermesh.mjs`, `handshake.mjs`, `framing.mjs`, `stream-socket-connection.mjs`) that runs `Server`/`Agent` connections over `@johnhenry/browsermesh-netway`'s `VirtualNetwork`/`StreamSocket` instead of a raw WebSocket, with per-agent Ed25519 identity (`@johnhenry/browsermesh-primitives`' `PodIdentity`) replacing the single shared `secret` — see the readme's new "Optional: the `dialback/browsermesh` transport" section for the handshake protocol and usage. Exposed via a new `"./browsermesh"` package-export subpath and a new `Agent` constructor option, `transport: (address) => Promise<Connection>`, that when provided is used instead of `new WebSocket(address)` — with no `transport` option, `Agent`'s behavior is unchanged (verified by re-running the full pre-existing test suite unmodified). `@johnhenry/browsermesh-netway` and `@johnhenry/browsermesh-primitives` are optional `peerDependencies`, never imported by `index.mjs`/`server.mjs`/`agent.mjs` — requiring plain `dialback` never touches either package.
...
- **`Promise.withResolvers()` (Node 22+ only) was used despite this package's own declared `engines.node` floor**: `util/invertedAsyncIterator.mjs`, `util/PromisesPlus.mjs`'s `withResolvers()` wrapper, and `server.mjs`'s `unBoundFetch()` all called it directly, which fails outright on Node 18/20. Replaced all three call sites with the portable manual-Promise-executor equivalent, restoring real compatibility with the Node versions this project actually claims to support. Surfaced immediately once CI was actually standing up a Node matrix rather than assumed to be fine.
- **`test:deno` failed on a clean checkout**: `deno.lock` references `npm:@types/node` (needed by Deno's Node-compat layer to type-check JSDoc referencing Node's `http`/`crypto` types), but it was never declared as a real `devDependency` -- it had only ever worked locally by accident, via unrelated global state on the machine running it. Declared it for real, pinned to the version `deno.lock` already expected, and verified against a genuinely empty `DENO_DIR`.
- **`test/browsermesh.test.mjs` failed outright on Node 18**, which has no global `crypto` object at all (`PodIdentity.generate()` threw `crypto is not defined`) -- a genuine WebCrypto Ed25519 platform floor for the optional identity-handshake layer, not a portability bug that a shim can paper over. Added `hasWebCryptoEd25519` feature-detection and scoped `{skip: ...}` to just the three test blocks that actually call `PodIdentity.generate()` (identity, handshake, e2e), leaving the rest of the suite -- including `FrameDecoder`, which needs no crypto -- running on every supported Node version. The core `dialback/browsermesh` transport itself isn't Node-18-incompatible; only this optional identity layer is.
