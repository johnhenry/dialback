...
- Two previously-unimplemented agent-selection strategies: `last-used` and `most-recent` (`first`, `last`, `random`, `round-robin` already existed). All six now have dedicated test coverage (`test/strategies.test.mjs`, 19 tests — this API had zero coverage before).
...
- Known limitation, documented rather than fixed in this pass: agent authentication is a single shared `secret` string, meaningfully weaker than e.g. `wsh`'s signed Ed25519 challenge-response handshake. Fine for the tunnel's current standalone scope; worth revisiting if this is ever used to expose real backends across a trust boundary.
